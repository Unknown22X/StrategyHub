...
from rangebot.domain.strategy_workflow import EntryExecutionSettings, StrategyExecutionPlan
...
class StrategyRuntimeRunner:
    ...
    def __init__(
        ...
        *,
        ...
        market_data_manager,
        order_manager,
        context_builder: Callable[[StrategyInstance], StrategyEvaluationContext] | None = None,
        execution_plan_resolver: Callable[[StrategyInstance], StrategyExecutionPlan | None] | None = None,
    ...
    async def _evaluate_instance(
        ...
        try:
            descriptor = self._registry.get(instance.type_id)
        ...
        if (
            self._last_event.get(instance.instance_id) == event_key
            or self._persisted_event_seen(instance.instance_id, event_key)
        ):
            return StrategyRuntimeOutcome(
                instance_id=instance.instance_id,
                status=instance.status,
        ...
        if instance.status == "monitoring" or evaluation.trade_request is None:
            return StrategyRuntimeOutcome(
                ...
                evaluated=True,
                submitted=False,
                reason="monitoring_only" if instance.status == "monitoring" else "no_trade",
            )
        ...
        try:
            ...
            execution_plan = (
                self._execution_plan_resolver(instance)
                if self._execution_plan_resolver is not None
                else None
            )
            entry = execution_plan.entry if execution_plan is not None else None
            order_type = entry.order_type if entry is not None else trade_request.order_type
            limit_price = (
            ...
            order_request = ManualOrderPreviewRequest(
                ...
                size_mode="margin",
                margin_amount=instance.requested_margin,
                leverage=instance.requested_leverage,
    ...
    def _resolve_limit_price(
        ...
        multiplier = Decimal("1") + percentage if match.group(2) == "+" else Decimal("1") - percentage
    ...
    def _event_key(cadence: str, context) -> str:
        if cadence == "closed_candle":
            if not context.candles:
                raise LookupError("No closed candles available for strategy evaluation.")
