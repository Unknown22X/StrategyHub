...
Issues and specs (you may know a spec as a PRD) for this repository live as
...
- **Frontier:** scan `.scratch/<effort>/issues/` for tickets that are open,
  unblocked, and unclaimed; first by number wins.
- **Claim:** set `Status: claimed` and save before any work.
- **Resolve:** append the answer under an `## Answer` heading, set
  `Status: resolved`, then append a context pointer (gist and link) to the
