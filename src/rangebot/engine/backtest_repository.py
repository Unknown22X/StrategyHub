...
class PortfolioBacktestRepository:
    ...
    def complete(
        ...
        with self._lock, Session(self._engine) as session:
            ...
            if (
                result.portfolio_request is None
                or result.portfolio_request.model_dump_json() != record.request_json
            ):
                raise ValueError(
                    "Backtest result configuration does not match the persisted run."
                )
            ...
            for index, item in enumerate(result.candidates):
                session.add(
                    BacktestCandidateRecord(
                        ...
                        sequence=index,
                        symbol=item.symbol,
                        candidate_json=item.model_dump_json(),
                    )
                )
            for index, item in enumerate(result.decisions):
                session.add(
                    BacktestDecisionRecord(
                        ...
                        sequence=index,
                        symbol=item.symbol,
                        decision_json=item.model_dump_json(),
                    )
                )
            for index, item in enumerate(result.orders):
                session.add(
                    BacktestOrderRecord(
                        ...
                        sequence=index,
                        symbol=item.symbol,
                        order_json=item.model_dump_json(),
                    )
                )
            for index, item in enumerate(result.fills):
                session.add(
                    BacktestFillRecord(
                        ...
                        sequence=index,
                        symbol=item.symbol,
                        fill_json=item.model_dump_json(),
                    )
                )
            session.commit()
            session.refresh(record)
    ...
    def record_input_fingerprint(
        ...
        with self._lock, Session(self._engine) as session:
            ...
            if record.input_data_hash not in {None, input_data_hash}:
                raise RuntimeError(
                    "Backtest input fingerprint is immutable once recorded."
                )
    ...
    def fail(
        ...
        *,
        ...
        with self._lock, Session(self._engine) as session:
            ...
            record.stage_message_ar = (
                "فشل Backtest. راجع المرحلة والسبب ثم أعد المحاولة."
            )
            record.failure_reason = reason[:4000]
            record.failure_code = code[:80]
            record.failure_stage = stage[:80]
            record.completed_at = datetime.now(UTC)
            session.commit()
            session.refresh(record)
            return self._to_domain(record)
    ...
    def cancel(self, backtest_id: str) -> StoredPortfolioBacktestRun:
        with self._lock, Session(self._engine) as session:
            ...
            if record.status in {"completed", "failed"}:
                raise RuntimeError(
                    f"Backtest {backtest_id} cannot be canceled after {record.status}."
                )
            record.status = "canceled"
            record.progress_percentage = min(record.progress_percentage, 99)
            record.stage_message_ar = "تم إلغاء Backtest. لم تُستخدم النتيجة الجزئية."
    ...
    def is_canceled(self, backtest_id: str) -> bool:
        with Session(self._engine) as session:
            return self._require(session, backtest_id).status == "canceled"
    ...
    def list(self, limit: int = 50) -> list[StoredPortfolioBacktestRun]:
        ...
        with Session(self._engine) as session:
            records = session.scalars(
                select(PortfolioBacktestRunRecord)
                .order_by(PortfolioBacktestRunRecord.created_at.desc())
                .limit(limit)
            )
        ...
        *,
        ...
        return StoredPortfolioBacktestRun(
            ...
            result=(
                ...
                if include_result and record.result_json
                else None
            ),
